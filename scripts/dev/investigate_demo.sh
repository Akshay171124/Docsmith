#!/usr/bin/env bash
# Free, local, end-to-end demo of `docsmith investigate` against a real Ollama
# model. Copies the sample fixture repo into a temp dir, makes a scripted
# breaking change to a documented function, and shows Docsmith flag the now-
# stale doc section — at $0, no API key required.
set -euo pipefail
...
echo "Running Docsmith investigate on a local Ollama model — $0, no API key needed."
...
python3 "$REPO_ROOT/docsmith.py" investigate \
    --repo "$WORK_DIR" \
    --base "$BASE" \
    --head "$HEAD" \
